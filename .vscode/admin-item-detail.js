...
async function loadDetail() {
    ...
    if (error || !item) {
        const reason = error ? `สาเหตุ: ${escapeHtml(error.message)}` : "ไม่พบข้อมูลของสิ่งของรายการนี้";
        detailContent.innerHTML = `<div class="detail-card"><div class="detail-body"><h2>ไม่สามารถโหลดรายละเอียดรายการได้</h2><p>${reason}</p></div></div>`;
        console.error("Load admin item detail failed:", error?.message);
        return;
    }
    ...
    detailContent.innerHTML = `
        <div class="detail-card">
            ...
            <div class="detail-body">
                ...
                <section class="info-section">
                    <h2><i class="fa-solid fa-shield-halved"></i> สถานะการยืนยันเจ้าของ</h2>
                    <div class="info-grid">
                        <div class="info-box"><div class="info-label">จำนวนครั้งที่ตอบไม่ถูกต้อง</div><div class="info-value">${item.claim_attempts || 0} / 3 ครั้ง</div></div>
                        <div class="info-box"><div class="info-label">ผลการตรวจสอบ</div><div class="info-value">${item.status === "claim_locked" ? "ระงับการยืนยันแล้ว" : "ยังสามารถยืนยันได้"}</div></div>
                        ${item.claim_locked_at ? `<div class="info-box full"><div class="info-label">เวลาที่ระงับ</div><div class="info-value">${new Date(item.claim_locked_at).toLocaleString("th-TH")}</div></div>` : ""}
                    </div>
                </section>
    ...
    const { data: attempts, error: attemptsError } = await supabase
        ...
        .select("id, attempt_no, attempted_at, claimant_id, submitted_answer")
        .eq("found_item_id", item.id)
        .order("attempted_at", { ascending: false });

    const attemptRows = (attempts || []).length
        ? attempts.map((attempt) => `<li>ครั้งที่ ${attempt.attempt_no} · คำตอบ: ${escapeHtml(attempt.submitted_answer || "-")} · ${new Date(attempt.attempted_at).toLocaleString("th-TH")}</li>`).join("")
        : (item.claim_attempts > 0
            ? `<li>พบการตอบผิดแล้ว ${item.claim_attempts} ครั้ง${item.claim_locked_at ? ` · ระงับเมื่อ ${new Date(item.claim_locked_at).toLocaleString("th-TH")}` : ""}${attemptsError ? " · ยังไม่ได้สร้างตารางประวัติรายครั้ง" : ""}</li>`
            : "<li>ยังไม่มีประวัติการตอบผิด</li>");

    detailContent.insertAdjacentHTML("beforeend", `
        ${item.claim_token ? `<section class="info-section"><h2><i class="fa-solid fa-qrcode"></i> QR สำหรับรับของ</h2><img src="https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(`CampusFind-UP-Claim:${item.claim_token}`)}" alt="QR สำหรับรับของ"><p>QR นี้หมดอายุ ${item.claim_expires_at ? new Date(item.claim_expires_at).toLocaleString("th-TH") : "ภายใน 24 ชั่วโมง"}</p></section>` : ""}
        <section class="info-section admin-controls">
            <h2><i class="fa-solid fa-gear"></i> การจัดการรายการ</h2>
        ...
        <section class="info-section">
            <h2><i class="fa-solid fa-triangle-exclamation"></i> ประวัติการยืนยันคำตอบผิด</h2>
    ...
    document.getElementById("deleteItemBtn").addEventListener("click", async () => {
        ...
        localStorage.removeItem("adminSelectedItemId");
